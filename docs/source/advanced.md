...
passed to every build command via `--target`. It encodes the board, the
FMC connector (where the board has more than one), and the line rate
where ambiguous:
...
token is taken as the *target board* and is what the build runner uses
...
line rate, FMC connector, GT lane mapping, etc.). The `build.py` runner
reads it directly at runtime, so the target list is never
hand-maintained.
...
Downstream PetaLinux / boot-image steps will pick up the new XSA
on the next build.
